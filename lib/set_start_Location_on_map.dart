import 'package:eshamba/set_end_location_on_map.dart';
import 'package:flutter/material.dart';
import 'package:google_geocoding/google_geocoding.dart';
import 'package:google_maps_flutter/google_maps_flutter.dart';
import 'dart:async';

class SetStartLocationOnMap extends StatefulWidget {
  const SetStartLocationOnMap(
      {Key? key, required this.latitude, required this.longitude})
      : super(key: key);
  final double latitude;
  final double longitude;
  @override
  State<SetStartLocationOnMap> createState() => _SetStartLocationOnMapState();
}

class _SetStartLocationOnMapState extends State<SetStartLocationOnMap> {
  final Completer<GoogleMapController> _controller = Completer();
  late LatLng selectedLocation = LatLng(widget.latitude, widget.longitude);
                ...
                onTap: () {
                  if (selectedLocation ==
                    ...
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => SetEndLocationOnMap(
                                  userLocation:
                                      LatLng(widget.latitude, widget.longitude),
                                  startLocation: selectedLocation,
                                  startAddress: selectedAddress.toString(),
                                )));
                  }
                },
                child: Container(
